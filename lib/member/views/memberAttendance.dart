...
class _MemberAttendanceState extends State<MemberAttendance> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text("Attendance"), backgroundColor: memberPrimary),
      body: Column(
        children: [
          ...
          Expanded(child: Consumer<MemberHomeController>(
            builder: (context, value, child) {
              return FutureBuilder(
                future: value.getAttendance(
                    date1: dateInput1.text, date2: dateInput2.text),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    return ListView.builder(
                        physics: const BouncingScrollPhysics(),
                        itemCount: snapshot.data[0]['attendancedata'].length,
                        itemBuilder: (context, index) {
                          /*     var data =
                              snapshot.data[0]['attendancedata']; */
                          return Card(
                            child: Padding(
                              ...
                              child: Column(
                                children: [
                                  Items(
                                      value: snapshot.data[0]['attendancedata']
                                          [index],
                                      titile: "Date"),
                                ],
                              ),
