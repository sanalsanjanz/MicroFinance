...
class MemberBankLoanPayment extends StatefulWidget {
  MemberBankLoanPayment({required this.unitid, required this.memberid});
  String unitid;
  String memberid;
  @override
  State<MemberBankLoanPayment> createState() => _MemberBankLoanPaymentState();
}

class _MemberBankLoanPaymentState extends State<MemberBankLoanPayment> {
  @override
  void initState() {
    super.initState();
      ...
      body: Consumer<MemberLoanController>(
        builder: (context, value, child) {
          return FutureBuilder(
            future: value.showMemberBankLoan(
                unitid: widget.unitid, memberid: widget.memberid),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (snapshot.data[0]['loandata'] != 'empty') {
