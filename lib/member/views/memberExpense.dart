...
class MemberExpense extends StatefulWidget {
  const MemberExpense({super.key});

  @override
  State<MemberExpense> createState() => _MemberExpenseState();
}

class _MemberExpenseState extends State<MemberExpense> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
          AppBar(title: const Text("Expense"), backgroundColor: memberPrimary),
      body: Column(
        children: [
          ...
          Expanded(child: Consumer<MemberHomeController>(
            builder: (context, value, child) {
              return FutureBuilder(
                future: value.getExpense(
                    date1: dateInput1.text, date2: dateInput2.text),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    return ListView.builder(
                        physics: const BouncingScrollPhysics(),
                        itemCount: snapshot.data[0]['expdata'].length,
                        itemBuilder: (context, index) {
                          var data = snapshot.data[0]['expdata'][index];
                          return Card(
                            child: Padding(
