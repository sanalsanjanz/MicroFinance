...
class MemberSessFund extends StatefulWidget {
  const MemberSessFund({super.key});

  @override
  State<MemberSessFund> createState() => _MemberSessFundState();
}

class _MemberSessFundState extends State<MemberSessFund> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text("Sess Fund"), backgroundColor: memberPrimary),
      body: Column(
        children: [
          ...
          Expanded(child: Consumer<MemberHomeController>(
            builder: (context, value, child) {
              return FutureBuilder(
                future: value.getSessFund(
                    date1: dateInput1.text, date2: dateInput2.text),
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    return ListView.builder(
                        physics: const BouncingScrollPhysics(),
                        itemCount: snapshot.data[0]['sessfunddata'].length,
                        itemBuilder: (context, index) {
                          var data = snapshot.data[0]['sessfunddata'][index];
                          return Card(
                            child: Padding(
